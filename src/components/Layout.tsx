...
import { Moon, Sun, Menu, X, MapPin, Compass, Briefcase, Crown, Smartphone, History } from "lucide-react";
...
const Layout = ({ children }: LayoutProps) => {
  ...
  const navItems = [
    ...
    { name: "Adventure", path: "/checkpoint-discovery", icon: <Smartphone className="h-5 w-5" /> },
    { name: "History", path: "/travel-history", icon: <History className="h-5 w-5" /> },
      ...
      <header className="glass sticky top-0 z-50 border-b border-border/50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <Link to="/" className="flex items-center space-x-2">
            <span className="text-2xl font-bold tracking-tight">Di&Choi </span><sub></sub>
          </Link>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-muted-foreground text-sm">
              © {new Date().getFullYear()} Di&Choi. All rights reserved. 
            </p>
